...
for k = 1:n-1
    ...
    for i = 1:robot_num
        ...
        if n_constraints == 0
            u = u_gtg;
        elseif n_constraints > 0
            u = quadprogram(x_i_k,x_hist(k,:), u_gtg, N_i_s);
            u = u';
        end
...
function [u,h_x]=quadprogram(x_i, x, u_gtg, N_i_s)
Q = 2 * eye(2); %Q 2x2
c = -2 * u_gtg'; %u_gtg 2x1
% H 4x2
...
h_x = function_h(x_i, x, N_i_s);
b = 10*h_x.^3;
u = quadprog(Q,c,H,b);
end

function h_x = function_h(x_i, x, N_i_s)
%h_x 4x1
